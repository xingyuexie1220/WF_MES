...
/// <summary>
/// DataGrid 本地化列（<see cref="LocDataGridTextColumn"/> / <see cref="LocDataGridEnumColumn"/>）。
/// <c>EnumMap</c> 须使用 <see cref="LocEnumMaps"/> 常量（已在 <see cref="LocEnum"/> 注册）。
/// </summary>
...
public class LocDataGridEnumColumn : LocDataGridTextColumn
{
    ...
    private void ApplyEnumBinding()
    {
        ...
        base.Binding = string.IsNullOrWhiteSpace(_enumMap) ? _valueBinding : LocEnumBinding.Create(_valueBinding, _enumMap);
    }
}

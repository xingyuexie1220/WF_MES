...
public class SysMenuController(IMenuService menuService) : WfApiControllerBase
{
    ...
    public async Task<ApiResult<MenuDto>> GetById(long id)
    {
        ...
        return menu is null ? ApiResult<MenuDto>.Fail("菜单不存在", 404) : ApiResult<MenuDto>.Ok(menu);
    }

...
public class SysDictController(IDictService dictService) : WfApiControllerBase
{
    ...
    public async Task<ApiResult<DictTypeDto>> GetTypeById(long id)
    {
        ...
        return item is null ? ApiResult<DictTypeDto>.Fail("字典类型不存在", 404) : ApiResult<DictTypeDto>.Ok(item);
    }
    ...
    public async Task<ApiResult<DictDataDto>> GetDataById(long id)
    {
        ...
        return item is null ? ApiResult<DictDataDto>.Fail("字典数据不存在", 404) : ApiResult<DictDataDto>.Ok(item);
    }

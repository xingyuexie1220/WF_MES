...
public class PositionService(ISqlSugarClient db) : IPositionService
{
    public async Task<PagedResult<PositionDto>> GetPagedListAsync(PositionQueryRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var items = await db.Queryable<SysPosition>()
            .LeftJoin<SysDept>((p, d) => p.DeptId == d.Id)
    ...
    {
        return await db.Queryable<SysPosition>()
            .LeftJoin<SysDept>((p, d) => p.DeptId == d.Id)
    ...
    {
        if (await db.Queryable<SysPosition>().AnyAsync(p => p.PositionCode == request.PositionCode && !p.IsDeleted))
        {
    ...
    {
        var position = await db.Queryable<SysPosition>().FirstAsync(p => p.Id == id && !p.IsDeleted)
    ...
    {
        if (await db.Queryable<SysUserPosition>().AnyAsync(up => up.PositionId == id))
        {
